...
<p>动作项为 <code>joint_pos</code>，维度 37。策略输出经 <code>scale=0.5</code> 缩放，并以默认关节位置为 offset，得到目标关节位置。Play 配置关闭观测 corruption（G1 训练配置本未启用随机推搡，Play 变体中相应事件同样为空），因此回放视频反映的是无感知噪声的策略表现。</p>
...
<h2 class="section-title"><span class="hnum">5</span>实验分析</h2>
  ...
  <p>第 6 节的消融结论来自训练期日志，反映的是学习过程的质量；本节用独立评估管线在训练之外考察训练所得策略的可靠性。评估协议借鉴 AGILE 工作流的分层设计——确定性场景提供低方差、可复现的回归基准，随机命令 rollout 检验命令分布下的鲁棒性——由四类场景组成，见表 8；其中固定网格的 10 个命令点覆盖训练命令范围 vₓ∈[0, 1.0] m/s、vᵧ∈[−0.5, 0.5] m/s、ωz∈[−1.0, 1.0] rad/s 及其组合。全部评估在去随机化的 Play 环境变体中进行（关闭训练时的观测噪声；G1 的训练配置本就未启用随机推搡，Play 变体中相应事件同样为空）；推理为确定性，即取动作分布的均值而不采样；六个策略在每个场景开始前重设同一评估种子，从而在相同的初始状态与命令序列下比较。每轮 50 个并行环境同时执行，逐回合明细与聚合结果落盘为 CSV 与 JSON，并记录 checkpoint 与场景配置的哈希以便溯源。</p>
  <table>
  ...
  <p>核心结果见表 9：两个条件呈现明确的取舍。<strong>跟踪精度上低熵条件更优</strong>——随机命令场景的线速度误差为 0.072 对 0.088 m/s，与第 6 节训练日志中低熵条件跟踪更优的总体方向一致，构成训练指标与独立评估的交叉验证；但收益的通道分布并不相同（训练日志中集中在 yaw，评估中集中在线速度），这与两处口径的差异有关——训练指标在带观测噪声的训练环境中按命令窗口统计，评估在无噪声环境下按回合统计。<strong>可靠性上默认熵条件全面占优</strong>：三个种子在全部四类场景、合计 31.7 小时仿真行走中零摔倒（MTBF ≥ 114,000 s）；低熵条件共摔倒 62 次（MTBF 1,819 s），且全部集中在 seed 42 一个策略——它在范围角点命令下（前进 1.0 m/s、横移 0.5 m/s、转向 1.0 rad/s 同时下达）50 回合全部摔倒（平均约 3.2 秒），在命令切换场景中另摔倒 12 次。值得注意的是，该种子同时是固定网格上跟踪误差最低的策略（线速度误差 0.066 m/s，六个策略中最低）——更紧的命令跟踪并未伴随更高的稳定性，二者在此呈现此消彼长的关系。</p>
  <table>
    ...
    <tbody>
      ...
      <tr><td rowspan="3">固定网格（10 命令点合并）</td><td>成功率</td><td><strong>1.000 ± 0.000</strong></td><td>0.967 ± 0.058</td></tr>
      <tr><td>摔倒数（/1,500 回合）</td><td>0</td><td>50（全部来自 seed 42 的范围角点命令）</td></tr>
  ...
  <p>逐命令点看，范围角点对两个条件都是误差最大的一点：默认熵条件虽未摔倒，线速度误差也升至 0.298 ± 0.033 m/s，为其余命令点（≤ 0.12 m/s）的两倍以上——三个命令通道同时达到训练范围上限时，策略以放宽跟踪精度来维持不摔倒。运动质量指标（关节加速度 RMS 约 105 rad/s²、动作变化率 RMS）在两条件间无显著差异。</p>
  <p>视频 3 展示了最佳候选策略（低熵 seed 43）在固定网格五个代表性命令点上的连续表现：0.5 与 1.0 m/s 直行、0.5 m/s 横移、1.0 rad/s 转向、范围角点，每段 8 秒。</p>
  ...
  <figure>
    ...
    <figcaption>图 10 · 固定网格集锦（视频 3）抽帧：四个命令点下的行走姿态，头顶箭头为环境自带的命令/实际速度可视化标记。视频与抽帧分辨率均为实测 1280×720。</figcaption>
  </figure>
  <p>如图 10 所示，该策略在四个命令点均保持直立行走，转向与横移时躯干和手臂有明显的补偿动作。视频 4 为命令切换场景的完整 60 秒回放（每 10 秒重采样一次命令）：该策略在多次命令切换之间保持行走，未出现摔倒。</p>
  ...
  <p>视频 5 记录了评估中发现的失效模式：低熵 seed 42 在范围角点命令下的回合。如图 11 的抽帧序列所示，机器人起步后约 2.6 秒身体前倾失稳，约 3 秒时躯干触地、回合终止（画面中机器人随后回到初始位置，为评估环境的自动重置）。</p>
  ...
  <p>独立评估对第 6 节的结论作出了补全：训练日志中低熵条件的回报优势，在评估中仅体现于跟踪精度；可靠性维度上默认熵条件严格占优，训练日志中 base_contact 终止率的微小差异（0.47% 对 0.57%）在评估中放大为 0 次对 62 次摔倒的差距。单一种子在极限命令下的系统性失效也表明，跨种子评估对于发现此类失效模式是必要的。</p>
  ...
  <p>本实验完成了 Unitree G1 在 Isaac Lab 平地速度跟踪任务上的可复现实验：使用官方环境配置和 PPO 训练预算，G1 可以在 1500 次迭代内学到稳定平地行走策略。多种子结果显示，降低熵系数可以提高当前预算下的末段回报和 yaw 跟踪质量；独立评估（第 7 节）进一步表明这一收益仅体现于跟踪精度——默认熵条件在 31.7 小时评估行走中零摔倒，而低熵条件的一个种子在极限命令下系统性摔倒。评估管线（固定命令网格、随机命令、耐力与命令切换四类场景）将作为后续实验的统一评估标准，奖励项消融将在同一协议下比较；随后进入粗糙地形、域随机化和抗扰动专项。</p>
