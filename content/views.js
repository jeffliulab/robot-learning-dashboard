...
<h2 class="section-title"><span class="hnum">5</span>实验分析</h2>
  ...
  <p>核心结果见表 9：两个条件呈现明确的取舍。<strong>跟踪精度上低熵条件更优</strong>——随机命令场景的线速度误差为 0.072 对 0.088 m/s，与第 6 节训练日志中低熵条件跟踪更优的总体方向一致，构成训练指标与独立评估的交叉验证；但收益的通道分布并不相同（训练日志中集中在 yaw，评估中集中在线速度），这与两处环境条件的差异有关——训练环境带观测噪声与随机推搡，评估环境将其全部关闭。<strong>可靠性上默认熵条件全面占优</strong>：三个种子在全部四类场景、合计 31.7 小时仿真行走中零摔倒（MTBF ≥ 114,000 s）；低熵条件共摔倒 62 次（MTBF 1,819 s），且全部集中在 seed 42 一个策略——它在范围角点命令下（前进 1.0 m/s、横移 0.5 m/s、转向 1.0 rad/s 同时下达）50 回合全部摔倒（平均约 3.2 秒），在命令切换场景中另摔倒 12 次。值得注意的是，该种子同时是固定网格上跟踪误差最低的策略（线速度误差 0.066 m/s，六个策略中最低）——更紧的命令跟踪并未伴随更高的稳定性，二者在此呈现此消彼长的关系。</p>
  <table>
