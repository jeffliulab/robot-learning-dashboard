...
Main: 第四篇实验报告「G1 速度阶梯：从步行到奔跑」——只改一项命令范围把 G1 从 1 m/s 步行训到 4 m/s 奔跑，以视频对比为核心。
Features:

1. G1 速度阶梯（自定义 cfg 子类仅覆盖 lin_vel_x 范围，奖励/观测/网络/超参全官方）：run2 (0–2.2) 在 2.0 m/s 三种子零摔、RMSE 0.040（官方范围基线 2/3 种子全摔）；sprint4 (0–4.5) 在 4.0 m/s 三种子零摔、双脚腾空 20.2%、60 秒耐力零摔——官方奖励表未改即自发涌现奔跑步态。
2. 视频对比为核心：速度阶梯三画面对照（1/2/4 m/s 同镜头并排）+ 命令范围边界失效对照（基线硬上 2.0 m/s 摔倒 vs run2 稳跑）+ 4.0 m/s 奔跑抽帧连拍（腾空相证据）。
3. 独立评估 9 策略 × 450 回合，按速度分档统计；新增 signature 图「速度阶梯命令范围示意」；训练回报对比曲线（run2/sprint4 各 3 种子）。
4. check_report 全绿；主站 :4321 目检通过。
