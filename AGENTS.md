...
⛔ **写或改任何报告正文之前，必须先读 STANDARD 规范链路，这是硬性前置步骤，不是参考建议**：从父仓读取 `../../../STANDARD.md`，按实验报告链路依次阅读**通用创作规范**（语域=教科书、禁口语化比喻、交稿清单）与 `STANDARDS/实验报告/实验报告模板.md`（章节骨架、图表编号、语言与配图规范），涉及机器人关节图时再读专项指南。写完对照通用交稿清单逐项自检。本仓不再保存规范副本。（背景：2026-07-11 曾有正文用了考试类口语比喻、与既有章节语域不符而返工——先读规范就不会发生。）
